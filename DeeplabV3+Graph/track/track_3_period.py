...
def process_predictResult(source_path,result_path,remove_edge=False):
    names = os.listdir(source_path)
    names = [name for name in names if '.tif' in name]

    if len(names)!=92:
        contrast_type=len(names)/92
        names=[name for name in names if int(name.replace(".tif",""))%contrast_type==0]
    names.sort()
...
def track_single_period(img_path,track_path,mask_path,remove_edge=True):

    predict_path = os.path.join(track_path, "predict_result")
    track_result_path = os.path.join(track_path, "track_RES")
    trace_path = os.path.join(track_path, "trace")
    createFolder(predict_path,clean=True)
    createFolder(track_result_path)
    createFolder(trace_path)

    process_predictResult(mask_path,predict_path,remove_edge=True)#二值化+过滤+去边界细胞+连通域编号
    predict_dataset_2(predict_path,track_result_path)#记得改输出的名字  GT：man_track***  RES:res_track
    get_trace(img_path, track_result_path, trace_path,text=True)#记得加上原图裁剪
    get_video(trace_path)

def track_all_periods(img_path,mask_dir,track_path):
